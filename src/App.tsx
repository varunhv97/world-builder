...
function App() {
  ...
  return <main className="app-shell"><header><p className="eyebrow">Local-first · autosaved</p><h1>World Builder</h1></header><section className="foundation-card"><canvas ref={canvasRef} className="world-canvas" onPointerDown={(event) => sculpt(event, event.shiftKey ? -35 : 35)} /><aside><h2>{paintMode ? 'Surface paint' : 'Terrain sculpt'}</h2><p>{paintMode ? 'Click terrain to cycle grass, water, and desert.' : 'Click to raise terrain. Shift-click to lower it.'}</p><button type="button" onClick={() => setPaintMode((value) => !value)}>{paintMode ? 'Use sculpt' : 'Use surface paint'}</button><button type="button" disabled={!undo.length} onClick={() => { const previous = undo.at(-1)!; setUndo((items) => items.slice(0, -1)); setRedo((items) => [...items, heights]); setHeights(previous) }}>Undo</button><button type="button" disabled={!redo.length} onClick={() => { const next = redo.at(-1)!; setRedo((items) => items.slice(0, -1)); setUndo((items) => [...items, heights]); setHeights(next) }}>Redo</button><button type="button" onClick={() => { setUndo((items) => [...items, heights]); setRedo([]); setHeights(generateTerrain(Date.now())) }}>Regenerate</button></aside></section></main>
}
export default App
