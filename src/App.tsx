...
function App() {
  ...
  return <main className="app-shell"><header><div><p className="eyebrow">Local-first · autosaved</p><h1>World Builder</h1></div></header><section className="workspace"><nav className="tool-rail" aria-label="Terrain tools"><Tool active={tool === 'raise'} label="Raise" icon="↑" onClick={() => setTool('raise')} /><Tool active={tool === 'lower'} label="Lower" icon="↓" onClick={() => setTool('lower')} /><Tool active={tool === 'paint'} label="Surface" icon="◒" onClick={() => setTool('paint')} /></nav><canvas ref={canvasRef} className="world-canvas" onPointerDown={edit} /><aside className="inspector"><p className="inspector-kicker">Active tool</p><h2>{toolCopy[0]}</h2><p>{toolCopy[1]}</p>{tool !== 'paint' && <label className="brush-control">Brush area <output>{brushRadius}</output><input aria-label="Brush area" type="range" min="1" max="6" value={brushRadius} onChange={(event) => setBrushRadius(Number(event.target.value))} /></label>}<div className="action-row"><button className="quiet-action" type="button" disabled={!undo.length} onClick={undoEdit}>Undo</button><button className="quiet-action" type="button" disabled={!redo.length} onClick={redoEdit}>Redo</button></div><button className="primary-action" type="button" onClick={() => { if (confirm('Regenerate terrain? This can be undone.')) { setUndo((items) => [...items, heights]); setRedo([]); setHeights(generateTerrain(Date.now())) } }}>Regenerate terrain</button></aside></section></main>
}
